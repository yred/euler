...
func solution() int {
	...
	// Assume that all numbers up to `limit` are Lychrel numbers
	count := limit
	...
	for a := 1; a <= limit; a++ {
		...
		for i := 1; i <= maxIterations; i++ {
			revA, _ := big.NewInt(0).SetString(common.ReverseString(bigA.String()), 10)

			bigA.Add(bigA, revA)

			if common.IsPalindrome(bigA.String()) {
				count--
				break
			}
		}
