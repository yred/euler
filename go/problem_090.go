...
func solution() (arrangements int) {
	squares := make([]common.Tuple, 9)
	for ix, n := 0, 1; n < 10; ix, n = ix+1, n+1 {
		squares[ix] = common.Tuple{n * n / 10, n * n % 10}
	}

	for _, c1 := range common.Combinations(common.Range(0, 10), 6) {
		d1 := digits(c1)

		for _, c2 := range common.Combinations(common.Range(0, 10), 6) {
			d2 := digits(c2)
...
func digits(elems []int) map[int]bool {
	set := common.GetSet(elems)
	if _, found6 := set[6]; found6 {
		set[9] = true
	} else if _, found9 := set[9]; found9 {
		set[6] = true
	}
	return set
}
