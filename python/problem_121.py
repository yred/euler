...
def winning_probability(turns):
    ...
    for red_count in range((turns+1)/2):
        for red_indexes in combinations(range(turns), red_count):
            probability += product(
                1 - blues[i] if i in red_indexes else blues[i]
...
def solution():
    return int(winning_probability(turns=15)**(-1))
