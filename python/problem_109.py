...
# Null, single, double and treble -- Types/multipliers
N, S, D, T = range(4)


def score(*darts):
    return sum(multiplier*value for multiplier, value in darts)


def solution():
    nullval = [(N, 0)]
    singles = [(S, i) for i in (range(1, 21) + [25])]
    doubles = [(D, i) for i in (range(1, 21) + [25])]
    trebles = [(T, i) for i in range(1, 21)]

    # All potential darts/outcomes
    darts = nullval + singles + doubles + trebles

    checkouts = defaultdict(int)

    for d in doubles:
        # Using `combinations_with_replacement` avoids processing "duplicates"
        for a, b in combinations_with_replacement(darts, 2):
            checkouts[score(a, b, d)] += 1

    return sum(times for score, times in checkouts.items() if score < 100)
