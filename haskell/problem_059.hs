...
solution :: Int
solution = sum . map ord . head . filter isEnglish $ map decrypt keys 

dictionary :: Set String
dictionary = fromList . words . unsafePerformIO $ readFile "../resources/english_common_10000.txt"

encrypted :: [Int]
encrypted = map read . splitOn ',' . unsafePerformIO $ readFile "../resources/p059_cipher.txt"

splitOn :: Eq a => a -> [a] -> [[a]]
splitOn d xs
    | null remaining = [first]
    | otherwise      = first : splitOn d (tail remaining)
...
isEnglish text = length english > length other
    where (english, other) = partition (`member` dictionary) . words $ map toLower text
