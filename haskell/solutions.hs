...
sumMulti3and5 n = sum [x | x <- [1..(n-1)], x `mod` 3 == 0 || x `mod` 5 == 0]

solution1 = sumMulti3and5 1000
...
evenFibNumbers a b maxi = evenFibNumbers' (sum [x | x <- [a, b], even x]) b (a+b) maxi
evenFibNumbers' s a b maxi = if b > maxi then s else evenFibNumbers' (s + (if even b then b else 0)) b (a+b) maxi

solution2 = evenFibNumbers 1 2 4000000
